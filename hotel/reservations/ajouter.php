    ...
    if (!$client_id || !$chambre_id || !$date_arrivee || !$date_depart) {
        ...
        if ($nb_nuits <= 0) {
            ...
            try {
                ...
                $success = "Réservation ajoutée. Total: " . formatMoney($prix_total) . " pour $nb_nuits nuits.";
...
<!DOCTYPE html>
<html lang="fr">
<head><meta charset="UTF-8"><title>Nouvelle réservation</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"><script>
...
<body>
    ...
    <div class="container mt-4">
        ...
        <form method="post" onchange="calculerPrix()">
            <div class="row">
                <div class="col-md-6 mb-3"><label>Client *</label><select name="client_id" class="form-control" required><option value="">-- Sélectionner --</option><?php foreach ($clients as $c): ?><option value="<?= $c['id'] ?>"><?= escape($c['prenom'] . ' ' . $c['nom']) ?></option><?php endforeach; ?></select></div>
                <div class="col-md-6 mb-3"><label>Chambre *</label><select name="chambre_id" id="chambre_id" class="form-control" required><option value="">-- Sélectionner --</option><?php foreach ($chambres as $ch): ?><option value="<?= $ch['id'] ?>" data-prix="<?= $ch['prix_nuit'] ?>"><?= escape($ch['numero']) ?> - <?= escape($ch['type']) ?> (<?= formatMoney($ch['prix_nuit']) ?>/nuit)</option><?php endforeach; ?></select></div>
                <div class="col-md-6 mb-3"><label>Arrivée *</label><input type="date" name="date_arrivee" id="date_arrivee" class="form-control" required></div>
                <div class="col-md-6 mb-3"><label>Départ *</label><input type="date" name="date_depart" id="date_depart" class="form-control" required></div>
                <div class="col-md-4 mb-3"><label>Statut</label><select name="statut" class="form-control"><option value="Confirmée">Confirmée</option><option value="En cours">En cours</option><option value="Terminée">Terminée</option><option value="Annulée">Annulée</option></select></div>
                <div class="col-md-4 mb-3"><label>Mode paiement</label><select name="mode_paiement" class="form-control"><option value="Espèces">Espèces</option><option value="Carte">Carte</option><option value="Virement">Virement</option><option value="Mobile Money">Mobile Money</option></select></div>
                <div class="col-md-4 mb-3"><div class="alert alert-info"><strong>Détails:</strong><br><span id="nuits_preview">-</span><br><strong>Total: <span id="total_preview">-</span></strong></div></div>
