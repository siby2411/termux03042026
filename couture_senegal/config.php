...
/**
 * Génère un lien direct vers WhatsApp (Gratuit et sans API)
 * @param string $telephone Numéro du client
 * @param string $message Message pré-rempli
 * @return string URL wa.me
 */
function lienWhatsApp(?string $telephone, string $message): string {
    if (!$telephone) return '#';
    
    // On ne garde que les chiffres
    $num = preg_replace('/[^0-9]/', '', $telephone);
    
    // Formatage Sénégal (77, 78, 76, 70...) -> Ajout de l'indicatif 221
    if (strlen($num) === 9) {
        $num = "221" . $num;
    }
