...
if ($exercice) {
    ...
    echo "<form method='post' action=''>
            <input type='hidden' name='id_exercice' value='$id_exercice'>
            <input type='submit' name='delete_bilan' value='Supprimer le Bilan d\'Ouverture' onclick=\"return confirm('Êtes-vous sûr de vouloir supprimer le bilan d\'ouverture ?');\">
          </form>";

    // Supprimer le bilan d'ouverture si le formulaire est soumis
    if (isset($_POST['delete_bilan'])) {
        $sql_delete = "DELETE FROM bilan_ouverture WHERE id_exercice = $id_exercice";
        if ($conn->query($sql_delete) === TRUE) {
            echo "Bilan d'ouverture supprimé avec succès.";
        } else {
            echo "Erreur lors de la suppression du bilan d'ouverture : " . $conn->error;
        }
    }
