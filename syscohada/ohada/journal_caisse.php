...
// Ajouter une entrée au journal de caisse
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ajouter'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO journal_caisse (date_operation, description, montant, type_operation) VALUES (?, ?, ?, ?)");
...
if (isset($_GET['supprimer'])) {
    $id = $_GET['supprimer'];
    $stmt = $conn->prepare("DELETE FROM journal_caisse WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->close();
}

// Récupérer les données du journal de caisse
$result = $conn->query("SELECT * FROM journal_caisse ORDER BY date_operation DESC");
...
<body>
    ...
    <h2>Liste des Opérations de Caisse</h2>
    <table border="1">
        <tr>
            <th>Date</th>
            <th>Description</th>
