...
<div class="container mt-5">
    ...
    <form action="insert_compte.php" method="GET">
        ...
        <div class="form-group">
            ...
            <select id="sous_classe" name="sous_classe" class="form-control" onchange="fetchComptesAndLibelles()">
...
<script>
    function fetchSousClasses() {
        ...
        if (classe_id) {
            ...
            $('#libelle').html('<option value="">Sélectionnez un Libellé</option>');
        }
    }

    function fetchComptesAndLibelles() {
        ...
        if (sous_classe_id) {
            ...
            $.get('fetch_comptes.php', { sous_classe_id: sous_classe_id }, function(data) {
                ...
                console.error("Erreur lors du chargement des numéros de compte:", xhr.responseText);
                alert('Erreur lors du chargement des numéros de compte');
            });

            // Charger les libellés
            $.get('fetch_libelles.php', { sous_classe_id: sous_classe_id }, function(data) {
                $('#libelle').html(data);
            }).fail(function(xhr, status, error) {
                console.error("Erreur lors du chargement des libellés:", xhr.responseText);
