...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'valider_inventaire') {
    ...
    if ($article) {
        ...
        // Récupération du dernier prix unitaire (CUMP ou prix défaut)
        ...
        // Insertion dans INVENTAIRE_PHYSIQUE
        ...
        $stmt2->execute([$article_id, $date_inventaire, $stock_theorique, $quantite_reelle, $valeur_ecart]);
        
        // Écriture comptable (compte 603)
        if ($ecart != 0) {
            $compte_stock = $article['compte_stock'];
            // Écart négatif : débit 603, crédit stock ; écart positif : débit stock, crédit 603
            if ($ecart < 0) {
                $sql_ecriture = "INSERT INTO ECRITURES_COMPTABLES (date_ecriture, libelle, compte_debite_id, compte_credite_id, montant, reference_piece, type_ecriture) VALUES (?, ?, 603, ?, ?, ?, 'INVENTAIRE')";
                $stmt3 = $pdo->prepare($sql_ecriture);
            ...
            $message = "✅ Inventaire validé - Écart : " . $ecart . " unités (" . number_format(abs($valeur_ecart), 0, ',', ' ') . " FCFA). Écriture comptable générée.";
        } else {
            $message = "✅ Inventaire validé - Aucun écart, pas d'écriture comptable.";
        }
    }
}
?>
<!-- Interface HTML (identique à avant) -->
...
<div class="col-md-4"><label>Article</label><select name="article_id" class="form-select"><?php foreach($articles as $a): ?><option value="<?= $a['id'] ?>"><?= $a['code_article'] ?> - <?= $a['libelle'] ?> (stock actuel: <?= $a['stock_actuel'] ?>)</option><?php endforeach; ?></select></div>
<div class="col-md-3"><label>Date inventaire</label><input type="date" name="date_inventaire" class="form-control" value="<?= date('Y-m-d') ?>"></div>
<div class="col-md-3"><label>Quantité réelle</label><input type="number" name="quantite_reelle" class="form-control" required></div>
<div class="col-md-2"><button type="submit" class="btn-omega mt-4">Valider</button></div>
</form>
</div></div></div></div>
