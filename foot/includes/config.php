...
define('DB_NAME', 'foot_school');
define('DB_USER', 'root');
define('DB_PASS', '');

// Configuration de l'application
define('APP_NAME', 'FOOT SCHOOL - Gestion des terrains et salles');
...
// Vérifier les alertes de stock pour l'admin (si la table livres existe)
if (isset($_SESSION['user_id']) && isAdmin()) {
    try {
        $pdo->query("SELECT 1 FROM livres LIMIT 1");
        $notificationSystem->checkStockAlert();
    } catch (PDOException $e) {
        // La table livres n'existe pas, ignorer
    }
}
// Pas de balise de fermeture PHP pour éviter les problèmes
