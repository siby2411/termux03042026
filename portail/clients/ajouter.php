...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nom || !$email) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO clients (type_client, nom, prenom, email, telephone, ville, adresse, mot_de_passe) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$type, $nom, $prenom, $email, $telephone, $ville, $adresse, $password]);
            $success = "Client ajouté avec succès. Mot de passe: client123";
...
<body>
    ...
    <div class="container mt-4">
        ...
        <form method="post">
            <div class="row">
                <div class="col-md-6 mb-3"><label>Type</label><select name="type_client" class="form-control"><option value="particulier">Particulier</option><option value="entreprise">Entreprise</option></select></div>
