...
$kpis = [
    ...
    'MargeBrute' => 0.00,
    'CoutDesVentes' => 0.00,
    'ResultatNet' => 0.00, // Doit être calculé
    'ROI_Estime' => 0.00
];
$frais_fixes_estimes = 15000.00; // Exemple de frais fixes

try {
    // 1. CALCULS DES KPI (CA, Marge Brute, Coût des Ventes)
    $query_kpi = "
        SELECT 
            COALESCE(SUM(DC.Quantite * DC.PrixVenteUnitaire), 0) AS CA_Total,
            COALESCE(SUM(DC.Quantite * DC.CUMP_Au_Moment_Vente), 0) AS CDV_Total
        FROM Commandes Cmd
        JOIN DetailsCommande DC ON Cmd.CommandeID = DC.CommandeID
        WHERE Cmd.DateCommande BETWEEN :date_debut AND :date_fin
    ";
    ...
    $kpis['MargeBrute'] = $kpis['ChiffreAffaires'] - $kpis['CoutDesVentes'];
    $kpis['ResultatNet'] = $kpis['MargeBrute'] - $frais_fixes_estimes;

    // Calcul du ROI (sur la Marge Brute avant frais fixes)
    ...
    // 2. CALCUL DU CUMP MOYEN ET ROI GLOBAL
    $cump_moyen = 0;
    ...
    // 3. DONNÉES POUR LES GRAPHIQUES (Tendance)
    $ca_daily_data = [];
    $marge_daily_data = [];
    $dates_labels = [];
    $query_daily = "
        SELECT 
            DATE(Cmd.DateCommande) AS Jour,
            COALESCE(SUM(DC.Quantite * DC.PrixVenteUnitaire), 0) AS CA_Jour,
            COALESCE(SUM((DC.Quantite * DC.PrixVenteUnitaire) - (DC.Quantite * DC.CUMP_Au_Moment_Vente)), 0) AS Marge_Jour
        FROM Commandes Cmd
        JOIN DetailsCommande DC ON Cmd.CommandeID = DC.CommandeID
        WHERE Cmd.DateCommande BETWEEN :date_debut AND :date_fin
        GROUP BY Jour
    ...
    // Initialisation des 30 jours pour éviter les trous
    ...
    // 4. DONNÉES POUR LES GRAPHIQUES (Répartition Marge)
    $produits_labels = [];
    $marges_produits_data = [];
    $query_marge_produit = "
        SELECT 
            P.Nom AS Produit,
            COALESCE(SUM((DC.Quantite * DC.PrixVenteUnitaire) - (DC.Quantite * DC.CUMP_Au_Moment_Vente)), 0) AS Marge_Total
    ...
    echo "<div class='alert alert-danger'>Erreur de chargement du Dashboard. Vérifiez les tables Commandes, DetailsCommande, Produits. " . $e->getMessage() . "</div>";
