...
<?php
// Calcul du Taux de Marge sur Coûts Variables (TMCV)
$tmcv = ($kpis['ChiffreAffaires'] > 0) ? ($kpis['MargeBrute'] / $kpis['ChiffreAffaires']) * 100 : 0;
?>
...
<div class="row">
    ...
    <div class="col-lg-3 col-md-6 mb-4">
        <div class="card border-0 shadow-lg text-dark bg-light">
            <div class="card-body">
                <div class="row">
                    <div class="col-8">
                        <div class="fs-6 text-uppercase">Taux Marge/Coûts Variables</div>
                        <div class="fs-3 fw-bold text-primary"><?= number_format($tmcv, 1, ',', ' ') ?> %</div>
                    </div>
                    <div class="col-4 text-end"><i class="fas fa-calculator fa-3x"></i></div>
                </div>
            </div>
        </div>
    </div>
...
<div class="row mt-5">
    <h2 class="mb-4"><i class="fas fa-balance-scale-right me-2 text-danger"></i> Hypothèses de Coûts et Résultat</h2>
    
    <div class="col-lg-6 mb-4">
        <div class="card shadow-lg h-100 border-danger border-3">
            <div class="card-header bg-danger text-white fw-bold">Analyse de la Rentabilité Nette (sur 30 jours)</div>
            <div class="card-body">
                <table class="table table-sm table-borderless">
                    <tr>
                        <th class="text-start">Marge Brute HT (MB)</th>
                        <td class="text-end fw-bold"><?= number_format($kpis['MargeBrute'], 0, ',', ' ') ?> €</td>
                    </tr>
                    <tr>
                        <th class="text-start text-danger">Charges Fixes Estimées (sur 30 jours)</th>
                        <td class="text-end fw-bold text-danger"><?= number_format($frais_fixes_estimes, 0, ',', ' ') ?> €</td>
                    </tr>
                    <tr class="table-active">
                        <th class="text-start">Résultat Net Estimé</th>
                        <td class="text-end fw-bold <?= ($kpis['ResultatNet'] >= 0) ? 'text-success' : 'text-danger'; ?>">
                            <?= number_format($kpis['ResultatNet'], 0, ',', ' ') ?> €
                        </td>
                    </tr>
                </table>
                <p class="text-muted small mt-2">
                    <i class="fas fa-info-circle"></i> Le Résultat Net montre si la Marge Brute a suffi à couvrir vos charges fixes de la période.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-6 mb-4">
        <div class="card shadow-lg h-100 border-primary border-3">
            <div class="card-header bg-primary text-white fw-bold">Coûts Variables et Marge (MCV)</div>
            <div class="card-body">
                <table class="table table-sm table-borderless">
                    <tr>
                        <th class="text-start">Chiffre d'Affaires</th>
                        <td class="text-end fw-bold"><?= number_format($kpis['ChiffreAffaires'], 0, ',', ' ') ?> €</td>
                    </tr>
                    <tr>
                        <th class="text-start text-warning">Coût des Ventes (CDV / Coûts Variables)</th>
                        <td class="text-end fw-bold text-warning"><?= number_format($kpis['CoutDesVentes'], 0, ',', ' ') ?> €</td>
                    </tr>
                    <tr class="table-active">
                        <th class="text-start">Marge sur Coûts Variables (TMCV)</th>
                        <td class="text-end fw-bold text-primary">
                            <?= number_format($tmcv, 1, ',', ' ') ?> %
                        </td>
                    </tr>
                </table>
                <p class="text-muted small mt-2">
                    <i class="fas fa-lightbulb"></i> Le TMCV est la part de chaque euro de CA qui contribue à payer les charges fixes et à générer du profit.
                </p>
            </div>
        </div>
    </div>
