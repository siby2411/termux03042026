<?php
// Inclure le fichier de connexion et démarrer la session si nécessaire
require_once '/config/db.php'; 
// Assurez-vous d'inclure ici votre fichier de vérification de session/authentification

// --- PARAMÈTRES ET CONSTANTES ---
// Définition des frais fixes mensuels pour le calcul du Résultat Net
$frais_fixes_mensuels = 15000.00; 

// Définition de la période actuelle (pour les indicateurs)
$date_courante = new DateTime();
$mois_en_cours_debut = $date_courante->format('Y-m-01');
$date_fin_courante = $date_courante->format('Y-m-d');


// --- 1. CALCUL DES AGRÉGATS FINANCIERS VIA V_Marge_Ventes ---

try {
    // 1.1. Calculer le CA, CDV et Marge Brute Totale
    $sql_kpi = "SELECT 
    ...
    $ca = (float)$kpi_data['TotalCA'];
    $cdv = (float)$kpi_data['TotalCDV'];
    $marge_brute = (float)$kpi_data['TotalMarge'];

    // 1.2. Calculs dérivés
    $resultat_net_est = $marge_brute - $frais_fixes_mensuels;
    $roi = ($cdv > 0) ? ($marge_brute / $cdv) * 100 : 0;

    // --- 2. CALCUL DE LA VALORISATION DU STOCK VIA V_Valorisation_Stock ---
    $sql_stock = "SELECT 
        ValeurTotaleStock 
    FROM 
    ...
    $valorisation_stock = (float)$stock_value;
