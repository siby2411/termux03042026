...
function log_mouvement($db, $id_piece, $quantite, $type, $ref_externe = null) {
    $query = "INSERT INTO MOUVEMENTS_STOCK (id_piece, date_mouvement, quantite_impact, type_mouvement, reference_externe) 
...
$query_mouvements = "
    SELECT 
        MS.date_mouvement, MS.quantite_impact, MS.type_mouvement, MS.reference_externe,
                        ...
                        <?php else: ?>
                            <?php foreach ($mouvements as $mouv): ?>
                                <tr>
                                    ...
                                    <td class="text-end"><?= format_qte($mouv['quantite_impact']) ?></td>
                                    <td>
                                        <span class="badge bg-<?= $mouv['quantite_impact'] > 0 ? 'success' : 'danger' ?>"><?= htmlspecialchars($mouv['type_mouvement']) ?></span>
                                    </td>
