...
if (!empty($search_date_debut) && !empty($search_date_fin)) {
    ...
    // Pas besoin de l'array $params ni $types ici, car la liaison sera explicite.
} else {
...
// 4. Préparer et exécuter la requête (Version sécurisée)
if (!empty($search_date_debut) && !empty($search_date_fin)) {
    // Exécution avec les 2 paramètres (recherche de disponibilité)
    $stmt = $conn->prepare($sql);
    
    // IMPORTANT: Liaison explicite des deux paramètres pour garantir l'ordre et le type "ss" (string, string)
    // L'ordre DOIT correspondre à l'ordre dans le SQL : [Search_Fin], [Search_Debut]
    if ($stmt) {
