...
$message = "";
$debug_sql = ""; // Variable pour stocker la requête de débogage
...
if (!empty($search_date_debut) && !empty($search_date_fin)) {
    ...
    if ($stmt) {
        // --- DÉBUT DU BLOC DE DÉBOGAGE SQL ---
        // Construction de la requête pour l'affichage, avec les paramètres insérés.
        // N'utilisez CECI QUE POUR LE DÉBOGAGE, PAS EN PRODUCTION DIRECTEMENT AVEC DES DONNÉES UTILISATEUR !
        $debug_sql_subquery = "SELECT DISTINCT voiture_id FROM locations WHERE (date_debut <= '" . $conn->real_escape_string($search_date_fin) . "' AND date_fin >= '" . $conn->real_escape_string($search_date_debut) . "') AND statut IN ('en_cours', 'reservee')";
        $debug_sql = "SELECT v.id, v.marque, v.modele, v.annee, v.couleur, v.prix_journalier, v.statut, p.nom AS partenaire_nom FROM voitures v LEFT JOIN partenaires p ON v.partenaire_id = p.id WHERE 1=1 AND v.id NOT IN (" . $debug_sql_subquery . ") ORDER BY v.marque, v.modele ASC;";
        // --- FIN DU BLOC DE DÉBOGAGE SQL ---
    ...
    // --- DÉBUT DU BLOC DE DÉBOGAGE SQL ---
    $debug_sql = $sql; // En mode sans dates, la requête est déjà complète
    // --- FIN DU BLOC DE DÉBOGAGE SQL ---
}
    ...
    <main>
        ...
        <?php if (!empty($debug_sql)): ?>
            <div style="background-color: #ffe0b2; border: 1px solid #ff9800; padding: 10px; margin-bottom: 20px; font-family: monospace; font-size: 0.9em; overflow-x: auto;">
                <strong>Requête SQL exécutée (pour le débogage):</strong><br>
                <pre><?php echo htmlspecialchars($debug_sql); ?></pre>
            </div>
        <?php endif; ?>
