...
if (!empty($search_date_debut) && !empty($search_date_fin)) {
    ...
    $sql .= " AND v.id NOT IN (
                ...
                FROM locations
                WHERE 
                    (date_debut <= ? AND date_fin >= ?) -- La période de recherche chevauche la location existante
                    AND statut IN ('en_cours', 'reservee') -- Considérer uniquement les locations actives
            )";
...
// 4. Préparer et exécuter la requête (Version sécurisée et robuste)
if (!empty($search_date_debut) && !empty($search_date_fin)) {
