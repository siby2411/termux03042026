...
// Vérifier les colonnes disponibles dans ventes
$columns = $pdo->query("DESCRIBE ventes")->fetchAll(PDO::FETCH_COLUMN);
$prix_col = 'prix_total';
if (in_array('total_ttc', $columns)) $prix_col = 'total_ttc';
elseif (in_array('montant_total', $columns)) $prix_col = 'montant_total';
elseif (in_array('total', $columns)) $prix_col = 'total';

$ventes_mensuelles = $pdo->query("
    SELECT 
        MONTH(date_vente) as mois,
        SUM($prix_col) as total_ventes,
        COUNT(*) as nb_ventes
    FROM ventes
...
$total_ventes = $pdo->query("SELECT SUM($prix_col) FROM ventes WHERE YEAR(date_vente) = $year")->fetchColumn() ?: 0;
...
// Vérifier les colonnes dans ventes_lignes
$lignes_columns = $pdo->query("DESCRIBE ventes_lignes")->fetchAll(PDO::FETCH_COLUMN);
$lignes_prix_col = 'prix_total';
if (in_array('total_ht', $lignes_columns)) $lignes_prix_col = 'total_ht';
elseif (in_array('montant', $lignes_columns)) $lignes_prix_col = 'montant';

$ventes_par_categorie = $pdo->query("
    SELECT 
        c.nom as categorie_nom,
        SUM(vl.$lignes_prix_col) as total_ventes
...
$top_clients = $pdo->query("
    SELECT 
        ...
        SUM(v.$prix_col) as total_achats
    FROM ventes v
