...
if (isset($_POST['add'])) {
    ...
    try {
        ...
        // --- INITIALISATION DU BULLETIN (Enveloppe) ---
        $annee = "2025-2026";
        ...
        // --- INITIALISATION DES NOTES (Détails) ---
        $query_init = "SELECT muv.matiere_id 
                       FROM programmes p 
                       JOIN matiere_uv muv ON p.uv_id = muv.uv_id 
                       WHERE p.classe_id = ?";
        $stmt_init = $conn->prepare($query_init);
        $stmt_init->bind_param("i", $_POST['classe_id']);
        $stmt_init->execute();
        $matieres = $stmt_init->get_result();

        $stmt_note = $conn->prepare("INSERT INTO notes (code_etudiant, id_matiere, annee_academique, note_cc1, note_cc2, note_exam) VALUES (?, ?, ?, 0, 0, 0)");
        while ($m = $matieres->fetch_assoc()) {
            $stmt_note->bind_param("sis", $code_etudiant, $m['matiere_id'], $annee);
            $stmt_note->execute();
        }
...
$etudiants = $conn->query("SELECT e.*, c.nom_class FROM etudiants e LEFT JOIN classes c ON e.classe_id = c.id ORDER BY e.id DESC")->fetch_all(MYSQLI_ASSOC);
        ...
        <div class="col-md-4">
            <div class="card p-3 shadow-sm border-0">
                ...
                <form method="post">
                    ...
                    <select name="classe_id" class="form-select mb-2" required>
        ...
        <div class="col-md-8">
            <table class="table table-hover align-middle">
                <thead class="table-dark">
                    <tr><th>Code</th><th>Nom & Prénom</th><th>Classe</th><th>Actions</th></tr>
                </thead>
                <tbody>
