...
// 1. Insertion des clients (100 clients)
echo "Insertion des clients...\n";
$clients = [];
$noms = ['DIOP', 'NDIAYE', 'FALL', 'SOW', 'BA', 'GUEYE', 'DIAGNE', 'SARR', 'MBAYE', 'LY', 'SECK', 'THIAM', 'DIALLO', 'TOURE', 'KANE'];
$prenoms = ['Amadou', 'Mariama', 'Ousmane', 'Aissatou', 'Ibrahima', 'Fatou', 'Mamadou', 'Aminata', 'Cheikh', 'Ndeye', 'Papa', 'Khadija'];
$villes = ['Dakar', 'Thiès', 'Saint-Louis', 'Touba', 'Ziguinchor', 'Kaolack', 'Mbour', 'Rufisque', 'Diourbel', 'Louga'];
...
for($i = 1; $i <= 50; $i++) {
    ...
    $sql = "INSERT INTO clients (numero_client, type_client, nom, prenom, email, telephone, ville, statut) 
            VALUES (:num, 'particulier', :nom, :prenom, :email, :tel, :ville, 'actif')";
    $stmt = $db->prepare($sql);
    $stmt->execute([
        ':num' => $numero_client,
        ':nom' => $nom,
        ':prenom' => $prenom,
        ':email' => strtolower($prenom . '.' . $nom . '@email.sn'),
        ':tel' => $telephone,
...
// Clients entreprises
$entreprises = ['OMEGA SARL', 'SENEGAL EXPRESS', 'DAKAR TRANS', 'AFRIQUE LOGISTICS', 'SUNU GROUPE', 'CARGILL SN', 'TOTAL SENEGAL', 'ORANGE SENEGAL'];
for($i = 1; $i <= 20; $i++) {
...
$marques = ['Toyota', 'Renault', 'Peugeot', 'Hyundai', 'Kia', 'Suzuki', 'Nissan', 'Honda', 'Mitsubishi', 'Volkswagen'];
$modeles = ['Corolla', 'Clio', '208', 'i10', 'Picanto', 'Swift', 'Micra', 'Civic', 'Lancer', 'Golf'];
...
foreach($clients as $client_id) {
    ...
    for($v = 0; $v < $nb_vehicules; $v++) {
        ...
        $sql = "INSERT INTO vehicules (immatriculation, marque, modele, annee_fabrication, energie, valeur_venale, proprietaire_id, statut) 
                VALUES (:immat, :marque, :modele, :annee, :energie, :valeur, :proprietaire, 'actif')";
        $stmt = $db->prepare($sql);
        $stmt->execute([
            ':immat' => $immat,
            ':marque' => $marque,
...
for($i = 1; $i <= 150; $i++) {
    ...
    $sql = "INSERT INTO contrats (numero_contrat, client_id, vehicule_id, formule, prime_nette, taxe, prime_ttc, mode_paiement, date_effet, date_echeance, statut) 
            VALUES (:num, :client, :vehicule, :formule, :prime_nette, :taxe, :prime_ttc, :mode, :date_effet, :date_echeance, 'actif')";
    $stmt = $db->prepare($sql);
    $stmt->execute([
        ':num' => $numero_contrat,
        ':client' => $client_id,
...
$types_sinistre = ['Accident', 'Vol', 'Incendie', 'Bris_de_glace'];
...
foreach($contrats as $contrat_id) {
    ...
    if($contrat) {
        ...
        // Premier paiement
        $numero_quittance = 'QUIT-' . date('Ymd') . '-' . $contrat_id;
