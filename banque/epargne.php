 ...
 * Gère l'ouverture et la liste des comptes.
 */

session_start();
require_once '../includes/db.php';
require_once '../includes/header.php'; 
// Assurez-vous d'avoir une fonction get_statut_color si vous l'utilisez
...
// Nécessaire pour les actions 'ouvrir' et 'edit'
$sql_clients = "
    SELECT 
        ClientID, 
        Nom, 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $action == 'ouvrir') {
    // 1. Récupération des données et échappement
    ...
    $Statut = 'Ouvert'; // Statut par défaut selon la table COMPTES
    
    // 2. Détermination du Taux d'intérêt (Simplifié)
    $taux = 0.00;
    ...
    // 3. Génération d'un CodeCompte unique
    $CodeCompte = 'CPT-' . $ClientID . '-' . mt_rand(100, 999);
    
    // 4. Exécution de la requête d'insertion
    if ($ClientID > 0) {
        $sql_insert = "
            INSERT INTO COMPTES (
        ...
        if ($conn->query($sql_insert)) {
            ...
            $action = 'list'; // Retour à la liste après succès
        } else {
